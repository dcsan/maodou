...
WechatAPI.prototype = {
  ...
  // wrap meteor get so first arg is params
  ...
  refreshAccessToken: function() {
    ...
    WechatTokens.saveAccessToken(this._app_id, result.data.access_token,
  ...
  getAccessTokenStr: function() {
    ...
    var token = WechatTokens.findOne({
      appId: appId
    });
    var accessToken = '';
    if (token == undefined) {
  ...
  sendMessageToUser: function(dataObject) {
    ...
    console.log("accessToken:" + JSON.stringify(dataObject, {
      indent: true
    }));
    ...
    HTTP.post(url, {
      ...
      if (result) {
        console.log("sendMessageToUser.result", JSON.stringify(result, {
          indent: true
        }));
      }
    });
